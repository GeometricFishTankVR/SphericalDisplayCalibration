...
namespace multi_proj_calib
{
	...
	{
	public:
		BlobPattern() :m_blob_detector(), m_blob_total_count(60), m_curr_cir(cv::Size(1,1)), m_grid_size(cv::Size(12, 10)) {}
		...
		cv::Point2f generateBlobGrid(const cv::Size& proj_img_size = cv::Size(setting::proj_width, setting::proj_height), const float blob_radius_pixel = setting::blob_radius);
